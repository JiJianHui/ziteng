...
function getHotelInfo2Rooms(hotelId)
{
   ...
   //根据hotelId 获取酒店信息的详细列表，包括 房间信息、图片信息
	...
	$.post(url,params,function(data){
		...
		if(json.success){
			...
			$("#hotel_roomzp").empty();	 //酒店房间列表 
			//$("#photo_flatitle").empty();	 //酒店图片列表标题 及数量 
			//增加一个富文本编辑器，将ho_box1、ho_box2、ho_box3、ho_box4、用一个字段来表示  t1_hotelContent
			
			 
			//$("#ho_box1").empty();	 //酒店设施服务 hotelServices
			//$("#ho_box2").empty();	 //酒店交通位置 addressDesc
			//$("#ho_box3").empty();	 //酒店介绍 hotelContent
			//$("#ho_box4").empty();	 //酒店特别提示  expandCol1
			var hotelDetail = json.datas.hotelInfo; // 获取到酒店信息列表
			var hotelPhotos = json.datas.hotelPhotos; // 获取到酒店图片列表
			var hotelRoomLs = hotelDetail.rooms; // 酒店房间列表
			...
			var $mbdhtxtdh = $(
			'<A title=酒店预订 href="home.html">酒店预订</A> '+
			'<A title="'+hotelDetail.ownArea+' 酒店预订" href="home-01.html?'+hotelDetail.ownArea+'&&&">'+hotelDetail.ownArea+'酒店</A>'+
			'<SPAN>'+hotelDetail.hotelName+'</SPAN>	'
			);
			var $topleftth = $(
			'<H1>'+hotelDetail.hotelName+' <SPAN class="star_img star_'+stari+'"></SPAN></H1>'+
			'<P>地址： '+hotelDetail.addressDesc+' <A class=thickbox '+
			'href="map.html?pos='+zuobiaolen+','+zuobiaolan+','+encodeURI(hotelDetail.hotelName)+','+encodeURI(hotelDetail.addressDesc)+'" '+
			'rel=nofollow target="_blank">电子地图</A> </P>'
			);
			var hpricetxt = '￥ ' + value + ' ';
			var redtxth = '<A title='+hotelDetail.hotelName+'团购 '+
			'href="#" target=_blank><SPAN class=tuan_icon></SPAN>仅售 '+value+' 元</A>';
			...
			$pingjcontent = $(
				...
				'<SPAN class="color_666 font_12">好评</SPAN></DIV>'+
		...
		//下面开始获取酒店客房的信息列表 
			...
			$("#hotel_roomzp").append($rooms);	 //酒店房间列表
...
//订单页面 获取 酒店信息 选择的房型信息 modify by 2014/6/3 参数为 hotelId、roomId
   ...
   //根据hotelId 获取酒店信息的详细列表，包括 房间信息、图片信息
	...
	$.post(url,params,function(data){
		...
		if(json.success){
			...
			var hotelPhotos = json.datas.hotelPhotos;	// 获取到酒店图片列表
			var hotelRoomLs = hotelDetail.rooms;		// 酒店房间列表
			var hotelStar = hotelDetail.hotelStar;		//获取酒店星级
			var value =  (hotelDetail.roomPrice==""?0:parseFloat(hotelDetail.roomPrice));	
			var stari = 1;				
			if (hotelStar != null && hotelStar != "undefined" && hotelStar != "")
			{
...
//根据房间id 获取房间的详细信息 回填 金额 和 左侧的 房间详情 modify by 2014/6/3
