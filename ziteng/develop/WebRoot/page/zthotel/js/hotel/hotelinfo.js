...
function hotelAreaInit()
{
	...
	$.post(url, function(data){
		...
		var hotelAreas = json.datas.hotelAreas;  //获取酒店地区列表
			$("#ownArea").empty();	
...
function hotelStarInit()
{
	...
	$.post(url, function(data){
		...
		var json = eval('('+data+')');
		if(json.success){  // 如果返回值 返回成功
		var hotelStars = json.datas.hotelStarLevs;  //获取酒店地区列表
			$("#hotelStar").empty();	
	...
	$.post(url, params,function(data){
		...
		if(json.success){
			...
			var $head = $('<tr class="title">'+
	                ...
	                '<td width="22%" align="center" class="title2">名称</td>'+
			...
			for(var i = 0; i < hotelList.length; i++){
				...
				var $row = $('<tr align="center" class="tdbg">'+
		                ...
		                '<td  valign="middle"><a href="hotel_room.html?id=' + sunTask.id + '"> 客房 </a>    <a href="hotel_edit.html?id=' + sunTask.id + '">修改 </a></td>'+
		              '</tr>');
			...
			$('#pageNo').html(json.datas.pageNo+'/' + currTotalPage  + "&nbsp;&nbsp;&nbsp;&nbsp;<a href='#' onclick='pre_page("+(json.datas.pageNo-1)+");'>上一页</a>&nbsp;&nbsp;<a href='#' onclick='next_page("+(json.datas.pageNo+1)+");'>下一页</a>");
