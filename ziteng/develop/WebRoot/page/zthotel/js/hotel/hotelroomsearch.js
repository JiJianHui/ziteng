...
function getSearchRoom(m_city,allInOne,m_preDate,m_endDate)
{
 ...
 $("#bjhotel").html(decodeURI(m_city)+'酒店');	
 $("#bjhotel01").html(decodeURI(m_city)+'酒店预订');	
 //以下 开始根据传输的条件过滤 
   //首选获取 酒店列表 然后获取酒店的客房	
	var url = "/ziteng/hotel/getSearchHotelInfosAndRooms.do";
	var hotelName = decodeURI(allInOne);
	if(decodeURI(allInOne) == "输入酒店名")
		hotelName = "";
	...
	$.post(url,params, function(data){
		...
		if(json.success){
			...
			var hotelList = json.datas.hotelInfos; // 获取到 查询的酒店信息列表
			$("#hoteldiv").empty();
			$("#prepagepre").empty();
			$("#pagepagetotal").empty();
			$("#nextpagenext").empty();
			...
			for(var i = 0; i < hotelList.length; i++)
			{
				...
				if (hotelStar != null && hotelStar != "undefined" && hotelStar != "")
				{
					...
					else if(hotelStar == "三星级")
					{
						stari = 6;
					}
					else if(hotelStar == "四星级")
					{
						stari = 8;
					}
					else if(hotelStar == "五星级")
					{
						stari = 10;
					}
					else if(hotelStar == "快捷酒店")
					{
						stari = 3;
					}
				}
				...
				'<DL class=ito_box>  <DT>酒店位置： </DT>'	+
				'<DD><A title='+hotelInfo.hotelAddress+' href="#">'+hotelInfo.hotelAddress+'</A> <A title='+hotelInfo.ownProvince+hotelInfo.ownArea+'酒店 href="#">'+hotelInfo.ownArea+'</A> </DD>'	+
				'<DIV class=htext_box2_jiag><DIV class=tuan_float></DIV><SPAN class=jiage>￥'+value+'</SPAN>起'+
				'<DIV class=hpl_cs>最近已有'+hotelInfo.reserveNum+'人预订</DIV><DIV class=hpl_cs><SPAN class=font_16b>'+hotelInfo.hotelGood+'</SPAN>好评 | '+(hotelInfo.hotelGood + hotelInfo.hotelBad)+'人评价</A></DIV></DIV>'+
				...
				'<TR sizcache="16" sizset="0"><Td width="26%">房型</TD><TD width="20%">早餐</TD><TD width="8%">床型</TD><TD width="8%">宽带</TD><TD width="16%">&nbsp;&nbsp;&nbsp;&nbsp;房价</TD><TD width="12%">房量</TD><TD width="10%">&nbsp;</TD></TR>' +
			...
			var pagedown= 
			'<LI class=pag_frist_bg><a class=page_01 href="#" onclick="pagepre(1);"><SPAN class=page_1></SPAN>首页 </A></LI>'+
			'<LI sizcache="13" sizset="28"><a class=page_02 href="#" onclick="pagepre('+ (json.datas.pageNo-1) +');" relid="2,16"><SPAN class=page_2></SPAN>上一页</A></LI>'+
			'<LI sizcache="13" sizset="36"><a class=page_03 href="#" onclick="pagenext('+ (json.datas.pageNo+1) +');" relid="2,16">下一页<SPAN class=page_3></SPAN></A></LI>'+
			'<LI sizcache="13" sizset="37"><a class=page_04 href="#" onclick="pagenext('+ pageNums +');" relid="360,16">末页<SPAN class=page_4></SPAN></A></LI>';
		
			/*var pagedown= 
			'<LI class=pag_frist_bg><a class=page_01 href="#" onclick="pre_page('+m_city+','+allInOne+','+m_preDate+','+m_endDate+',1);"><SPAN class=page_1></SPAN>首页 </A></LI>'+
			'<LI sizcache="13" sizset="28"><a class=page_02 href="#" onclick="pre_page('+m_city+','+allInOne+','+m_preDate+','+m_endDate+','+ (json.datas.pageNo-1) +');" relid="2,16"><SPAN class=page_2></SPAN>上一页</A></LI>'+
			'<LI sizcache="13" sizset="36"><a class=page_03 href="#" onclick="next_page('+m_city+','+allInOne+','+m_preDate+','+m_endDate+','+ (json.datas.pageNo+1) +');" relid="2,16">下一页<SPAN class=page_3></SPAN></A></LI>'+
		...
		//alert("ad");
			...
			$("#nextpagenext").html('<A href="#" onclick="pagenext('+ (json.datas.pageNo+1) +');" rel=nofollow>下一页</A>');
   ...
   //首选获取 酒店列表 然后获取酒店的客房	
	...
	$.post(url,function(data){
		...
		if(json.success){
			...
			var hotelList = json.datas.hotelInfos; // 获取到 查询的要首页显示的酒店信息列表
			...
			if(totalNum > 0) //如果总数量 大于 1 则 更新#hotelbob1的数据内容
			{
				...
				if(hotelRoomsNum > 4) //如果 酒店房间类型 数量 超过 4 则 只取前四个 如果 小于 四 则去实际的length值
				{
					hotelRoomsNum = 4;
				}
				...
				{
					var roomInfo = hotelRoomsInfo[k];  // 酒店客房的详细信息
			...
			if(totalNum > 2) //如果总数量 大于 3 则 更新#tuanbox1的数据内容
			{
				...
				'<p class="hot_title">'+
				'	<a href="home-011.html?id=' + hotelRInfo.id + '">'+hotelRInfo.hotelName+'标准大床房/标准双床房</a>'+
				'</p>'+
				'<p class="hotel_money">团购价:<span class="col_org">￥<span class="t16">'+value+'</span></span></p>'
				);
			...
			if(totalNum > 3) //如果总数量 大于 4 则 更新#tuanbox2的数据内容
			{
				...
				$tuanbox2 = $(
					...
					'</dt>'+
					'<dd>团购价：<span class="col_org bold t16">￥'+(parseFloat(value) * parseFloat(valueZk))+'</span> </dd>'+
					'<dd>原 价：<span class="col_6">￥'+hotelRInfo.roomPrice+'</span> </dd>'+
					'<dd>折 扣：<span class=" col_org">'+hotelRInfo.expandCol5+'折</span> </dd>'+
					'</dl>'
				);
