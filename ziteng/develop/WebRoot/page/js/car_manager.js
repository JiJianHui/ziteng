...
function add_car(){
	...
	if(name==null || name == undefined || name =="" ||name.length>20){	alert("车辆名称不能为空,且长度在10位内"); return false; }
	if(carDegreeId == null || carDegreeId == undefined){ alert("请选择车辆档次"); return false;}
	if(price==null || price == undefined || price == "" || !unsigned_int_reg.test(price)){alert("请输入正数价格"); return false;};
	if(volume==null || volume == undefined || volume == "" || !unsigned_int_reg.test(volume)){alert("请输入正数可载数量"); return false;};
