...
function add_or_modify_guide(){
	...
	if(name==null || name==undefined || name==""){ alert("攻略名称不能为空");return false; }
	if(priceWayId==null || priceWayId==undefined || priceWayId==""){ alert("计价类型不能为空");return false; }
	if(departAreaId==null || departAreaId==undefined || departAreaId==""){ alert("出发区域不能为空");return false; }
	if(arriveAreaId==null || arriveAreaId==undefined || arriveAreaId==""){ alert("到达区域不能为空");return false; }
...
function save_car_degree_price(){
	...
	if(price ==null || price == undefined || price == ""){ alert("请输入价格"); return false; }
	if(!  /^[1-9]\d*$/.test(price)){alert("请输入正整数价格"); return false;}
...
function addGuide2CarDegreePrice(){
	...
	var resultInfo = isModifyGuide ? "修改成功": "添加成功";
...
//        	 var json = eval("(" + data +")");
        	 var msg = isModifyGuide ? "修改成功": "添加成功";
        	 alert(msg);
...
function loadGuideInfoToModify(){
	$("#lg_add_or_modify_type").html("修改攻略信息");
	$("#btn_add_or_modify").html("保存攻略信息");
