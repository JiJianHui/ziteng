	...
	if(changePasswordSta == 0)
	{
		...
		(
			...
			function(result)
			{
				...
				$("#codeBox input[name='member_code']").val('Please type in the 验证码');
...
$(function()
{
	...
	(
		function ()
		{
			...
			if(member_name =="")
			{
				...
				accountObj.parent().append("<a class='text-error'>Please type in the 用户名</a>");
				return false;
			}	
			else
			{
			...
			if(member_password =="")
			{
				...
				password.parent().append("<a class='text-error'>Please type in the Password</a>");
				return false;
			}	
			else
			{
			...
			$.post(
				...
				function(result)
				{
					...
					{
						removeBlackBox();
						password.parent().append("<a class='text-error'>Password错误</a>");
						return false;
					}
	...
	(	
		function()
		{
			if($(this).attr('name').indexOf('head') < 0 && $("#cardtype").val() == "ID")
			{
