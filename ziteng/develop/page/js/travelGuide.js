...
//		removeBlackBox();
		...
		if(json.success){
			...
			for(var i = 0; i < guides.length; i++){
	            ...
	            '</div>'+
	            '<div class="post-ding">'+
	            '<input type="button" value="Join Now" onclick="goto_details('+guide.id+')" style="width:80px; height:30px;margin:30px 0 0 -20px;" id="anticipate" class="btn btn-primary"/>'+
	           '</div>'+
	            ...
	            '<div class="post-author">'+
	                ...
	                '<a></a>Comments：'+guide.latestComment+'</a></a>'+
	                '<span class="comment-date">'+guide.latestCommentTimeString+'</span></span>'+
	            '</div>'+
	            '<div class="post-content">'+ guide.detail +
	            '</div>'+
	            '<span class="status"><i class="icon_view"></i>参与次数：'+guide.countOrder+'<i class="icon_comment"></i>Comments次数：'+guide.commentsCount+'</span>'+
	            '</li>';
