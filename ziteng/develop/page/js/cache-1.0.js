...
(function(w) {
	...
	cacheMgr.webSQL.createTable = function(tableName, callback) {
		...
		cacheMgr.webSQL.db
				&& cacheMgr.webSQL.db.transaction(function(transaction) {
	...
	cacheMgr.webSQL.insertData = function(tableName, data, suc, err) {
		...
		t.db && t.db.transaction(function(transaction) {
			transaction.executeSql(sql, data, function() {
				...
				err && err({
					code : cacheErr.code.WEBSQL_ERR,
					msg : cacheErr.msg.WEBSQL_ERR
				});
			});
		}, function() {
			err && err({
				...
				&& cacheMgr.webSQL.db.transaction(function(transaction) {
					transaction.executeSql(sql, [], function(ts, results) {
						if (results.rows.length > 0) {
							...
							err && err({
								code : cacheErr.code.NO_RESULT,
								msg : cacheErr.msg.NO_RESULT
							});
						}
					}, function(ts, e) {
		...
		for (var i = 0; i < t.num && !isErr; i++) {
			...
			t.fetch(name, function() {
				...
				if (!runErr) // 只执行一次err（）
				{
					runErr = true;
					err && err({
						code : cacheErr.code.FILESYSTEM_ERR,
						msg : cacheErr.msg.FILESYSTEM_ERR
					});
				}
			});
		}
			...
			fileEntry.createWriter(function(writer) {
				writer.onwriteend = function() {
					fileEntry.createWriter(function(write) {
						...
						if (!BlobBuilder) {
							bb = new Blob([ JSON.stringify(t.cache[name]) ], {
								type : "text/plain"
							});
							write.write(bb);
						} else {
	...
	cacheMgr.fileSystem.getItem = function(key, suc, err) {
		...
		if (t.cache[name] != undefined && t.cache[name] != null
				&& t.cache[name] != "") {
			t.get(key, suc, err);
		} else {
			...
			request.onsuccess = function(e) {
				...
				if (iDB.db.version != "1.0") {
					...
					requestVersion.onsuccess = function(event) {
						...
						// 创建table
						var objectStore = iDB.db.createObjectStore(
								iDB.defaultTableName, {
									keyPath : "key",
									autoIncrement : false
								});
	...
	cacheMgr.indexedDB.setItem = function(key, value, suc, err) {
		if (!cacheMgr.indexedDB.inited) {
			err && err({
				code : cacheErr.code.INDEXDB_ERR,
				msg : cacheErr.msg.INDEXDB_ERR
			});
			return;
		}
		...
		if (cacheMgr.indexedDB.IDBTransaction.READ_WRITE)
			trans = db.transaction([ table ],
					cacheMgr.indexedDB.IDBTransaction.READ_WRITE);
		else
	...
	cacheMgr.localStorage.setItem = function(key, value, suc, err) {
		try {
			...
			err && err({
				code : cacheErr.code.LOCALSTORAGE_ERR,
				msg : cacheErr.msg.LOCALSTORAGE_ERR
			});
		}
 ...
 */
function cache_request(url, params, result_function) {
	alert("cache_request");
	if (navigator.onLine) { // 在线状态
		...
		$.post(url, params, function(data) {
			...
			//storage url+params/ data
			var key = url + (params == null ? "" : JSON.stringify(params));
			key = escape(key);
			ziteng_cache.setItem(key, escape(data), function() {
				console.log("success to setItem");
			}, function() {
				console.log("fail to setItem");
			});
		});
	} else {// 离线状态. read from cache.
		alert("off line");
		var key = url + (params == null ? "" : JSON.stringify(params));
		key = escape(key);
		ziteng_cache.getItem(key, function(data) {
			console.log("success to getItem");
			result_function(unescape(data));
		}, function() {
			console.log("fail to getItem");
		});
	}
}
...
function putItem(key, value){
	ziteng_cache.setItem(key, value, function() {
		console.log("success to setItem");
	}, function() {
		console.log("fail to setItem");
	});
