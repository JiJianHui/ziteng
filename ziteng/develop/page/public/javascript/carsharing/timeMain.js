 ...
 * @ignore
 */
var lzcTime,oTimer,activity;
var timeMonth,oYear,newTimeP,newTimeN;
...
Calendar.prototype = {
    ...
    */
    _setPos: function (triggerNode) {
        ...
        this.container.style.left = (l-7) + "px"
    },
    /**
    * 添加事件
            ...
            if (oTarget.tagName.toUpperCase() === "A" && oTarget.className != "disabled") {
                ...
                if (that.triggerNode.tagName.toUpperCase() === "INPUT") {
                    if (lzc.id(that.id).className == 'time-end') {
                        ...
                        if (that.isMore) {
                            var d = that.triggerNode.value, v = oTarget["data-date"];
                            if (d) d = d.split(','); else d = [];
                            var index = (function () {
                            ...
                            if (index == -1) d.push(v); else d.splice(index, 1);
                            d = d.join(',')
                            that.triggerNode.value = d;
                            if (that.isDateInfo) that.setDateInfo(d);
                        } else {
