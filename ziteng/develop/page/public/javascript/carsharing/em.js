...
$.fn.extend({
    mface: function (options) {
        if ($('#style_comment_face').length == 0) {
            var style = [
                    '<style type="text/css" id="style_comment_face">',
                        '#face { position: absolute; display: none; background: #fff none repeat scroll 0 0; border: 1px solid #CCCCCC; padding: 0px 0px; z-index: 999999; text-align: left; margin: 0px 0px; }',
                        '#facebox { margin:5px; border-top: 1px solid #e8e8e8; border-left: 1px solid #e8e8e8; width: 372px; }',
                        '#texttb { height: 20px; margin: 0px 0px; padding: 0; }',
                        '#texttb .f_close { position: absolute; right: 4px; top: 2px; z-index: 99; }',
                        ...
                        '.f_close { background: url("/images/icon/delete_items.gif") no-repeat scroll transparent; cursor: pointer; display: block; height: 19px; width: 19px; }',
                        '.arrow { background: url("/images/icon/layer_arrow.png") no-repeat scroll 0 0 transparent; display: block; overflow: hidden; position: absolute; }',
        ...
        var defaults = {
            ...
            containerObj: '', //表情框父对象
            emotions: em, //表情信息json格式，id表情排Number phrase表情使用的替代短语url表情文件名
            imageurl: window.domain + '/res/images/em/',
            top: 0, //相对偏移
            left: 0 //相对偏移
        };
        ...
        return this.each(function () {
            ...
            $(Obj).bind("click", function (e) {
                ...
                faceHtml += '<div id="texttb"><a class="f_close" title="Close" href="javascript:void(0);"></a></div>';
                ...
                container.find("#face_detail ul >li").bind("click", function (e) {
                    ...
                    var faceText = txt;
                    var oldVal = options.txtAreaObj.val();

                    options.txtAreaObj.hideTip();
                    options.txtAreaObj.val(oldVal + faceText);
                    container.find("#face").remove();
    ...
    em2Html: function () {
        return this.each(function () {
            ...
            var regx = /\[[A-Za-z\u4e00-\u9fa5]+\]/g; //正则查找“[]”格式
            var rs = rContent.match(regx);
            if (rs) {
                for (i = 0; i < rs.length; i++) {
                    for (n = 0; n < em.length; n++) {
                        if (em[n].phrase == rs[i]) {
                            var t = "<img src='" + window.domain + "/res/images/em/" + em[n].url + "' />";
                            rContent = rContent.replace(eval('/\\' + rs[i] + '/g'), t);
                            break;
                        }
                    }
                }
