...
function initHotelRoom(hotelId)
{
   // alert(hotelId + "--ada");		
	...
	$.post(url,params, function(data){
		...
		var json = eval('('+data+')');
		if(json.success){  // 如果返回值 返回成功
		var hotelTypes = json.datas.hotelRooms;  //获取Hotel地区列表
			$("#roomName").empty();	
			for(var i = 0; i < hotelTypes.length; i++){
				var sunTask = hotelTypes[i];
...
function getHotelOrder(hotelId)
{
	...
	if(hotelId != null)
	{
		...
		//给导航赋值hotelid		
		$('#hotel_room_bar').html('<a href="hotelorder_add.html?id='+hotelId+'">添加Hotel Order</a>&nbsp;|&nbsp;<a href="hotel_order.html?id='+hotelId+'">管理Hotel Order</a>&nbsp;|&nbsp;<a href="hotel_order.html?id='+hotelId+'">查询Hotel Order</a>&nbsp;|&nbsp;<a href="#" onclick="refreshSMSReplay();">刷新</a>');
	...
	$.post(url,params, function(data){
		...
		if(json.success){
			...
			var $head = $('<tr class="title">'+
	                ...
	                '<td width="10%" align="center" class="title2">订单Title</td>'+
					'<td width="15%" align="center" class="title2">客人姓名</td>'+
				    '<td width="15%" align="center" class="title2">Phone</td>'+
	                '<td width="8%" align="center" class="title2">Check in</td>'+
	                '<td width="8%" align="center" class="title2">Check out</td>'+
					'<td width="8%" align="center" class="title2">Room numbers</td>'+
	                '<td width="7%" align="center" class="title2">Status</td>'+
	                '<td width="8%" align="center" class="title2">Order Time</td>'+
			...
			for(var i = 0; i < hotelList.length; i++){
				...
				var $row = $('<tr align="center" class="tdbg">'+
						...
						'<td  valign="middle"><a href="hotelorder_edit.html?id=' + sunTask.id + '&hotelId='+sunTask.hotelId+'">Change </a></td>'+
		              '</tr>');
...
function getMyHotelOrder()
{
	...
	$.post(url,params, function(data){
		...
		if(json.success){
			...
			$('#pageInfo').append(json.datas.total+'条，Each page'+json.datas.pageSize+'条');
			$('#pageInfo').append(json.datas.pageNo+'/' + currTotalPage1);
			$('#pageInfo').append("&nbsp;&nbsp;&nbsp;&nbsp;");
			$('#pageInfo').append("<a href='#' onclick='pre_page1("+(json.datas.pageNo-1)+");'>Previous</a>");
			$('#pageInfo').append("&nbsp;&nbsp;&nbsp;&nbsp;");
			$('#pageInfo').append("<a href='#' onclick='next_page1("+(json.datas.pageNo+1)+");'>Next</a>");
