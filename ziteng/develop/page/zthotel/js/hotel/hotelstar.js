...
{	
	var obj=document.getElementsByName('hotelCheckId');  //选择所有name="'test'"的对象，返回数组    
	  ...
	  var params = {"starContents":str};
	  var url = "/ziteng/hotel/hotelStarLevsDelete.do";
...
*Hotel typeChange
*/
function hotelStarEdit(id)
{
	
	var typeId = id;
	var hotelStar = $("#ls_areaName" + id).val();
	
	var order = $("#ls_order" + id).val();
	
	var params = {"id":typeId,"hotelStar":hotelStar,"order":order};
	
	
	var url = "/ziteng/hotel/hotelStarLevUpdate.do";
	$.post(url,params,function(data){
		var json = eval('('+data+')');
		if(json.success){			
...
function hotelStarAdd()
{
	...
	var params = {"hotelStar":hotelStar,"order":order};
	
	
	var url = "/ziteng/hotel/hotelStarLevAdd.do";
	$.post(url,params,function(data){
		var json = eval('('+data+')');
		if(json.success){			
			alert("添加成功！"+json.msg);
...
function gethotelStar()
{
	...
	$.post(url, function(data){
		...
		if(json.success){
			//将返回的数据放到html中
			var hotelList = json.datas.hotelStarLevs;
			$("#hotel_main").empty();	
			...
			for(var i = 0; i < hotelList.length; i++){
				...
				var $row = $('<tr align="center" class="tdbg">'+
		                ...
		                '<td align="center"  valign="middle"><input name="ls_areaName'+sunTask.id+'" type="text" class="input" id="ls_areaName'+sunTask.id+'" size="25" value="'+sunTask.hotelStar+'" maxlength="20"></td>'+		 		               
		                ...
		                '<td  valign="middle"><input type="Button" id="hotelStarEdit'+sunTask.id+'" name="hotelStarEdit'+sunTask.id+'" value="修  改" class="bt" onclick="hotelStarEdit('+sunTask.id+')"></td>'+
		              '</tr>');
			...
			var $rowpage = $('<tr class="title">'+
	                ...
	                '<td colspan=2 align="left" valign="top"><input name="areaDelete" id="areaDelete" type="button" class="bt" value="删 除" onClick="hotelStarDelete()"></td>'+
	                '<td id="pageInfo" ></td>'+
